...
**핵심 개념:**
- SM 기본 세팅: 좌 250만 / 우 250만 / 몸PV 0만
  → 몸PV가 자동으로 좌,우 중에 작은 쪽으로 합산: (좌,우 중 작은 쪽) + 몸pv = 250만 → SM 달성
- 이 몸PV를 언제 어떻게 넣느냐에 따라 상위 DM 수당이 달라짐
...
PROCESS:
  ...
  4. 각 SM의 타겟 배열 세팅:
     ...
     - 우 타겟: 250만 (SM의 기본 소실적) / 150만 (SSM 기본)
     - 몸PV 풀: 0만 (SM) / 0만 (SSM)
```
...
```
DM 현황 (22일):
  좌: 60만 PV, 우: 60만 PV → 현재 15점 (30만/30만 구간)
  → 70만/70만 달성 시 30점 가능
  → 필요: 좌 +10만, 우 +10만

몸PV 배분:
  22일에 몸PV 20만 투입 (좌/우 각 10만)
  → DM 22일: 좌 70만, 우 70만 → 30점 달성
  → SM 몸PV 잔여: 40만 - 20만 = 20만 남음

직급표 기록:
  SM | 22일 | 좌: 0 | 우: 0 | 몸PV: 20만
```
